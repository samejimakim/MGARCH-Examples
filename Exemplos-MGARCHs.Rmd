...
$$
\Sigma_t=\textbf{A}_0 \textbf{A}_0^T + \textbf{A}_1 \epsilon_{t-1}^T \epsilon_{t-1} \textbf{A}_1^T + \textbf{B}_1\Sigma_{t-1}^T
$$

em que $\textbf{A}_0$ é uma matriz triangular inferior de tal maneira que $\textbf{A}_0\textbf{A}_0^T$ é positiva deifinida.   
A partir de tal formulação, considere os seguintes valores das matrizes:   
...
utilizaremos os pacotes `MTS` e `mgarch` para estimar os parâmetros deste processo.
